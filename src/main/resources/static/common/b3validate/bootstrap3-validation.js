 ...
 * ========================================================= */
!function($) {
    $.fn.validation = function(options,callback) {
   ...
   $.fn.validation.defaults = {
        validRules : [
            ...
            {name: 'nonninteger', validate: function(value) {return (!/^\d+$/.test(value));}, defaultMsg: '请输入正整数数字或0。'},
            ...
            {name: 'ennumberx',validate:function(value){return(!/^[a-zA-Z0-9_]+$/.test(value));},defaultMsg:"必须是英文字母、数字及下划线组成。"}
        ],
        reqmark:true,
        iFocus:false,
    ...
    var validateField = function(field, valid) { 
        ...
        var dataUrl=(el.attr('data-url')?el.attr('data-url'):null); //
        var confirmPassword=(el.attr('confirmPassword')?el.attr('confirmPassword'):null);
        var msg = null;
        ...
        if (confirmPassword && !error){
        	error = !(el.val()==$("#password").val());
        	if(error && (msg==null || errorMsg=='')){
        		errorMsg = "两次输入的确认密码不一致，请重新输入";
        	}
        }
        ...
        if (!error && dataUrl != null && dataUrl != ""){
            ...
            $.post(dataUrl,{iptObj :$(field).val()},function(data){
            	var json = eval('(' + data + ')');
   	         	params.err = !json.success;
   	         	params.msg = json.msg;
   	        }).error(function() { 
   	        	params.err = true;
   	         	params.msg = "数据校验失败！";
        ...
        if(form){
            ...
            if(fstyle == 0){
                ...
                //alert(el.parents('.controls').find(".help-block"));
            	el.parents('.controls').find(".help-block").html(errorMsg);
            }
            else if(fstyle == 1){

            }
            else if (fstyle == 2){
            	if(!globalOptions.iFocus && controlGroup.attr('class').indexOf('has-success') == -1){
            		el.focus();
            		globalOptions.iFocus = true;
            	}
                //controlGroup.find("#valierr").remove();
                //el.parent().after('<span class="help-block" id="valierr">' + errorMsg +'</span>');
                //alert(el.parents('.controls').find(".help-block").html());
            	if(controlGroup.attr('class').indexOf('has-success') >=0){
            		errorMsg = "通过验证";
            	}
                el.parents('.controls').find(".help-block").html(errorMsg);
            }
